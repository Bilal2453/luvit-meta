...
---@field logLevel? discordia.enusms # The level to use for logging (see [Enumerations](https://github.com/SinisterRectus/Discordia/wiki/Enumerations)).<br>**Default:** `enums.logLevel.info`.
---@field dateTime? # The date and time format to use logging.<br>**Default:** `'%F %T'`.
---@field syncGuilds? # Whether to automatically sync all guilds on start up (user-accounts only).<br>**Default:** `false`.
---@field gatewayIntents? # Represents which intents to use. The default includes all non-privileged intents.<br>**Default:** `3243773`.
